...
class OmniVoiceGenerationConfig:
    ...
    # Use deterministic position selection by default in the Comfy node.
    # The original stochastic setting can produce clearly wrong utterances
    # for identical prompts across runs.
...
class OmniVoice(PreTrainedModel):
    ...
    def __init__(self, config: OmniVoiceConfig, llm: Optional[PreTrainedModel] = None):
        ...
        self.normalized_audio_codebook_weights = [w / sum(config.audio_codebook_weights) for w in config.audio_codebook_weights]
        ...
        try:
            ...
            if not train_mode:
                if not os.path.isdir(pretrained_model_name_or_path):
                    raise ValueError("omnivoice_comfy expects a local OmniVoice snapshot directory.")
                ...
                if not os.path.isdir(audio_tokenizer_path):
                    raise ValueError("OmniVoice snapshot is missing the audio_tokenizer directory.")
    ...
    def load_asr_model(self, model_name: str = "openai/whisper-large-v3-turbo"):
        raise RuntimeError("Whisper auto-transcription is disabled in omnivoice_comfy. Provide ref_text for voice cloning.")
    ...
    def generate(
        ...
        **kwargs,
        ...
        if self.audio_tokenizer is None or self.text_tokenizer is None:
            raise RuntimeError(
                "Model is not loaded with audio/text tokenizers. Make sure you loaded the model with OmniVoice.from_pretrained()."
            )
    ...
    def create_voice_clone_prompt(
        ...
        if self.audio_tokenizer is None:
            raise RuntimeError("Audio tokenizer is not loaded. Make sure you loaded the model with OmniVoice.from_pretrained().")
        ...
        if ref_text is None:
            raise ValueError("omnivoice_comfy requires ref_text for voice cloning. Whisper auto-transcription is not supported.")
        ...
        ref_audio_tokens = self.audio_tokenizer.encode(
            ref_wav.unsqueeze(0).to(device=self.audio_tokenizer.device, dtype=tokenizer_dtype),
        ).audio_codes.squeeze(0)  # (C, T)
        if ref_audio_tokens.size(-1) == 0:
            raise ValueError("Reference audio produced zero tokenizer frames. Use a longer speech clip or disable preprocess_prompt.")
    ...
    def _decode_and_post_process(
        ...
        if isinstance(tokens, list):
            chunk_audios = [self.audio_tokenizer.decode(t.to(tokenizer_device).unsqueeze(0)).audio_values[0].cpu() for t in tokens]
            audio_waveform = cross_fade_chunks(chunk_audios, self.sampling_rate)
        else:
            audio_waveform = self.audio_tokenizer.decode(tokens.to(tokenizer_device).unsqueeze(0)).audio_values[0].cpu()
    ...
    def _post_process_audio(
        ...
        """
        raw_generated_audio = generated_audio
        if postprocess_output:
            ...
            if generated_audio.size(-1) == 0 and raw_generated_audio.size(-1) > 0:
                logger.warning("Postprocessing removed the entire generated waveform; falling back to the untrimmed audio.")
                generated_audio = raw_generated_audio
    ...
    def _generate_chunked(self, task: GenerationTask, gen_config: OmniVoiceGenerationConfig) -> List[List[torch.Tensor]]:
        ...
        for i in range(task.batch_size):
            ...
            text_chunk_len = int(gen_config.audio_chunk_duration * self.audio_tokenizer.config.frame_rate / avg_tokens_per_char)
        ...
        assert all(has_ref) or not any(has_ref), (
            "Chunked inference requires all items to either have or not have ref_audio. Mixed ref/non-ref is not supported."
        )
    ...
    def _preprocess_all(
        ...
        if voice_clone_prompt is not None and (ref_text is not None or ref_audio is not None):
            logger.warning("Both voice_clone_prompt and ref_text/ref_audio are provided. ref_text/ref_audio will be ignored.")
    ...
    def _prepare_inference_inputs(
        ...
        style_tokens = (
            self.text_tokenizer(style_text, return_tensors="pt").input_ids.repeat(self.config.num_audio_codebook, 1).unsqueeze(0)
    ...
    def _generate_iterative(self, task: GenerationTask, gen_config: OmniVoiceGenerationConfig) -> List[torch.Tensor]:
        ...
        progress_node_id = getattr(self, "_comfy_progress_node_id", None)
        progress_bar = ProgressBar(gen_config.num_step, node_id=progress_node_id) if ProgressBar is not None else None
        ...
        for step in range(gen_config.num_step):
            ...
            if progress_bar is not None:
                progress_bar.update_absolute(step + 1)
